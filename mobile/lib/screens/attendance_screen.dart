...
class _AttendanceScreenState extends State<AttendanceScreen> {
  ...
  Future<void> _executeAction(String action) async {
    ...
    try {
      ...
      if (action != 'Register') {
        final locResponse = await http.get(Uri.parse('${ApiConfig.baseUrl}/attendance/office-location'));
        if (locResponse.statusCode != 200) throw Exception("Failed to fetch office location.");
        final locData = jsonDecode(locResponse.body);
        final officeLat = locData['latitude'];
        final officeLng = locData['longitude'];
        final maxDistance = locData['maxDistanceMeters'];
        ...
        double distanceInMeters = Geolocator.distanceBetween(position.latitude, position.longitude, officeLat, officeLng);

        if (distanceInMeters > maxDistance) {
          throw Exception("You are not near the office location. (${distanceInMeters.toInt()}m away, max $maxDistance\m)");
        }
      }
